...
import { PrivateKey, Aes, ChainTypes, ChainStore, FetchChain,
...
export type TxCreate = (job: Job, tr: TransactionsModel,
                        tx) => Promise<Decimal | null>;

export async function txIssue(job: Job, tr: TransactionsModel,
                              tx): Promise<Decimal> {
  ...
  tx.add_type_operation('asset_issue', { issuer: assetTo.get('issuer'),
    asset_to_issue: { amount: amountTo.toNumber(),
...
export async function txBurn(job: Job, tr: TransactionsModel,
                             tx): Promise<null> {
  ...
  const amountFrom = Decimal.mul(tr.amountFrom!, Decimal.pow(10,
    assetFrom.get('precision'))
  );

  tx.add_type_operation('asset_reserve', {
    amount_to_reserve: { amount: amountFrom.toNumber(),
                         asset_id: assetFrom.get('id') },
    payer: assetFrom.get('issuer'), fee: { amount: 0, asset_id:
                                           assetFee.get('id') }
  });
...
export async function txCommit(
  job: Job, tr: TransactionsModel, txCreate: TxCreate,
  ...
  if(tr.status === previousStatus || tr.status === errStatus) {
    ...
    const chainProperties = await Apis.instance()
      .db_api().exec('get_global_properties', []);
...
export async function processTx(
  job: Job, tr: TransactionsModel, tx, txIndex: number,
  ...
  const elapsed = now > tx.expiration ? now - tx.expiration
                + chainProperties.parameters.maximum_time_until_expiration
                : tx.expiration - now;
  const broadcast = chainProperties.parameters.maximum_time_until_expiration
                  > elapsed;
  let blockTo;
  ...
  do {
    ...
    do {
      ...
      if(broadcast === true && await Apis.instance().db_api()
        .exec('get_recent_transaction_by_id', [tx.id()]) === null) {
        try {
          await tx.broadcast();
        } catch(error) {
          console.error(`Job ${job.id} failed broadcast transaction ${tx.id()}`,
                        error);
        }
      }
      ...
      blockFrom = blockFrom
               ?? (tryFetchNumber < appConfig.bithsaresBlockTryCheckNumber
                   ? tr[txPrefix].blockFrom : 0);
      blockTo = dynamicChainProperties.head_block_number;

      for(blockHeight = blockTo; blockHeight >= blockFrom! &&
          founded === false; blockHeight--) {
        ...
        for(const txCheckRaw of block.transactions) {
          ...
          try {
            ...
            console.error(`Job ${job.id} skipped transaction ${txCheck}`,
                          error);
      ...
      if(founded === false) {
        if(++tryFetchNumber >= appConfig.bithsaresBlockTryCheckNumber) {
          console.error(`Job ${job.id} skipped transaction ${tx.id()}: ` +
                        `unknown, pending`);
    ...
    const result = await sequelize.transaction(async transaction => {
      ...
      if(txInit === true || txCommited.confirmations !== confirmations) {
...
async function skipOrProcessTx(
  job: Job, tr: TransactionsModel, tx, previousStatus: StatusInitial,
  commitPrefix: TransactionsModelCommitPrefix, blockFrom: number
): Promise<boolean> {
  for(let txIndex = 0; txIndex < tx.operations.length; txIndex++) {
    ...
    if(op[0] === ChainTypes.operations.transfer) {
      ...
      if(op[1].amount.asset_id !== assetFrom.get('id') ||
      ...
      const result = await processTx(job, tr, tx, txIndex, previousStatus,
                                     commitPrefix, blockFrom);
...
async function fetchAllHistoricalBlock(
  ...
  for(let blockHeight = blockTo; blockHeight >= 0; blockHeight--) {
    ...
    for(const txRaw of block.transactions) {
      ...
      const result = await skipOrProcessTx(job, tr, tx, previousStatus,
                                           commitPrefix, blockHeight);
...
async function fetchAllNewBlock(job: Job, tr: TransactionsModel,
  ...
  while(true) {
    ...
    } while(blockFrom >= dynamicChainProperties.last_irreversible_block_num
                       + 1);
...
export async function fetchBlockUntilTxFound(
  ...
  return await resolveAny(job, [
    ...
    { task: async state => { return await fetchAllNewBlock(
        job, tr, dynamicChainProperties.last_irreversible_block_num + 1,
        previousStatus, commitPrefix, state
      ); }, skip: false }
  ]);
};
